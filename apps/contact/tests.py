...
class ContactViewsTests(TestCase):
    ...
    def test_contact_view(self):
        ...
        self.assertTemplateUsed(response, 'contact/contact.html')  # Verify that the template used is 'contact/contact.html'

    def test_contact_message_creation(self):
        """Test that a contact message can be created correctly."""
        self.client.login(username='regularuser', password='password123')  # Log in as a regular user
        ...
        self.assertEqual(response.status_code, 302)  # Check for redirection after submission
        self.assertTrue(ContactMessage.objects.filter(email='test@example.com').exists())  # Check if the message was saved
    ...
    def test_contact_message_without_name(self):
        ...
        self.assertEqual(response.status_code, 200)  # Check that the form returns the same page
        self.assertFormError(response, 'contact_form', 'name', 'This field is required.')  # Verify form error message
    ...
    def test_contact_message_without_email(self):
        ...
        self.assertEqual(response.status_code, 200)  # Check that the form returns the same page
        self.assertFormError(response, 'contact_form', 'email', 'This field is required.')  # Verify form error message
    ...
    def test_contact_message_without_subject(self):
        ...
        self.assertEqual(response.status_code, 200)  # Check that the form returns the same page
        self.assertFormError(response, 'contact_form', 'subject', 'This field is required.')  # Verify form error message
    ...
    def test_contact_message_without_message(self):
        ...
        self.assertEqual(response.status_code, 200)  # Check that the form returns the same page
        self.assertFormError(response, 'contact_form', 'message', 'This field is required.')  # Verify form error message
